...
$data_undangan = mysqli_query($mysqli, $sql);
...
if (isset($_POST['btn_delete'])) {
    if (isset($_POST['selected'])) {
        foreach ($_POST['selected'] as $selectedid) {
            ...
            $deleteData = "DELETE FROM form WHERE id_form =" . $selectedid;
                    ...
                    <div class="col-md-9">
                        <div class="card card-primary card-outline">
                            <div class="card-header">
                                ...
                                <div class="card-tools">
                                    <form action="mailbox.php" method="get">
                                        <div class="input-group input-group-sm">
                                            <button type="submit" value="Refresh Page" class="btn btn-default btn-sm" onClick="document.location.reload(true)">
                                                <i class="fas fa-sync-alt"></i>
                                            </button>
                                            <input type="text" class="form-control" name="search" value="<?= @$search ?>" placeholder="Cari Undangan">
                                    ...
                                    <div class="table-responsive mailbox-messages">
                                        <table class="table table-hover table-striped">
                                            ...
                                            <tbody>
                                                ...
                                                $jumlah_data = mysqli_num_rows($data_undangan);
                                                $total_halaman = ceil($jumlah_data / $batas);

                                                $data = mysqli_query($mysqli, $sql . " ORDER BY start_event ASC limit $halaman_awal, $batas");
                                                $nomor = $halaman_awal + 1;
                                                $no = 0;
                                                while ($form = mysqli_fetch_array($data)) {
                                                ?>
                                                    <tr>
                                                        <td>
                                                            <a href="delete.php?id_form=<?php echo $form['id_form']; ?>" onclick="return confirm('Anda yakin mau menghapus item ini ?')" class="link">
                                                                <i class="far fa-trash-alt"></i>
                                                            </a>
                                                        </td>
                                                        </td>
                                                        <td class="mailbox-subject"><b><?= $form['nama_instansi'] ?></b> - <?= $form['nama_pj'] ?>
                                                        <td class="mailbox-name">
                                                            <a href="readmail.php?id_form=<?= $form['id_form']; ?>">
                                                                <?= $form['nama_event'] ?>
                                                            </a>
                                                        </td>
                                                        <td class="mailbox-date"><?= $form['start_event'] ?></td>
                                                        <td class="mailbox-date"><?= $form['end_event'] ?></td>
                                                        <td>
                                                            <label>
                                                                <a class="btn btn-primary" href="accept.php?id=<?= $form['id_form'] ?>" role="button">Setuju</a>
                                                            </label>
                                                            <br>
                                                            <label>
                                                                <a class="btn btn-primary btn-danger" href="decline.php?id=<?= $form['id_form'] ?>" role="button">Tolak</a>
                                                            </label>
                                                        </td>
                                                    </tr>
                                                <?php } ?>
