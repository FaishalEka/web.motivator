...
$sql = "INSERT INTO riwayat
        ...
        SELECT '' as id
            ...
            , file
        FROM terkonfirmasi
        WHERE DATE(end_event) < CURDATE()
    ";
$result = mysqli_query($mysqli, $sql);
if ($result) {
    # HAPUS EVENT YANG SUDAH DIPINDAHKAN
    $sql = "DELETE FROM terkonfirmasi WHERE DATETIME(end_event) < CURRENT_DATETIME()";
    mysqli_query($mysqli, $sql);
}

# BARU LAKUKAN QUERY UNTUK MENAMPILKAN TERKONFIRMASI
$sql = "SELECT * FROM terkonfirmasi";
$listTerkonfirmasi = $mysqli->query($sql);
...
// mengambil data undangan
$data_terkonfirmasi = mysqli_query($mysqli, $sql);
// menghitung data undangan
                        ...
                        <div class="card">
                            ...
                            <div class="card-body p-0">
                                <ul class="nav nav-pills flex-column">
                                    ...
                                    <li class="nav-item">
                                        <a href="form_decline.php" class="nav-link">
                    ...
                    <div class="col-md-9">
                        <div class="card card-primary card-outline">
                            <div class="card-header">
                                ...
                                <div class="card-tools">
                                    <form action="terkonfirmasi.php" method="get">
                                        <div class="input-group input-group-sm">
                                            <button type="submit" value="Refresh Page" class="btn btn-default btn-sm" onClick="document.location.reload(true)">
                                                <i class="fas fa-sync-alt"></i>
                                            </button>
                                            <input type="text" class="form-control" name="search" value="<?= @$search ?>" placeholder="Cari Disetujui">
                                    ...
                                    <div class="table-responsive mailbox-messages">
                                        <table class="table table-hover table-striped">
                                            ...
                                            <tbody>
                                                ...
                                                $jumlah_data = mysqli_num_rows($data_terkonfirmasi);
                                                $total_halaman = ceil($jumlah_data / $batas);

                                                $data = mysqli_query($mysqli, $sql . " ORDER BY start_event ASC limit $halaman_awal, $batas");
                                                $nomor = $halaman_awal + 1;
                                                $no = 0;
                                                while ($terkonfirmasi = mysqli_fetch_array($data)) {
                                                ?>
                                                    <tr>
                                                        <td>
                                                            <a href="deleteconfirm.php?id=<?php echo $terkonfirmasi['id']; ?>" onclick="return confirm('Anda yakin mau menghapus item ini ?')" class="link">
                                                                <i class="far fa-trash-alt"></i>
                                                            </a>
                                                        </td>
                                                        <td class="mailbox-subject"><b><?= $terkonfirmasi['nama_instansi'] ?></b> - <?= $terkonfirmasi['nama_pj'] ?>
                                                        </td>
                                                        <td class="mailbox-name">
                                                            <a href="readconfirm.php?id=<?= $terkonfirmasi['id']; ?>">
                                                                <?= $terkonfirmasi['title'] ?>
                                                            </a>
                                                        </td>
                                                        <td class="mailbox-date"><?= $terkonfirmasi['start_event'] ?></td>
                                                        <td><?= $terkonfirmasi['end_event'] ?></td>
                                                    </tr>
                                                <?php } ?>
                                            </tbody>
                                        </table>
